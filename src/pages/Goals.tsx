...
export default function Goals() {
  ...
  useEffect(() => {
    if (!goalsLoading && realGoals.length > 0) {
          ...
          >
            <div className="p-5 rounded-sm border bg-card">
              ...
              <p className="text-lg font-light tabular-nums">
                {summaryData.totalProgress.toFixed(1)}%
              </p>
              <div className="h-1.5 bg-secondary rounded-full overflow-hidden mt-2">
                <div
                  className="h-full rounded-full bg-success transition-all"
                  style={{
                    width: `${Math.min(summaryData.totalProgress, 100)}%`,
                  }}
                />
              </div>
            </div>
            <div className="p-5 rounded-sm border bg-card">
              ...
              <p className="text-lg font-light tabular-nums">
                {formatCurrency(summaryData.totalCurrent)}
              </p>
            </div>
            <div className="p-5 rounded-sm border bg-card">
              <p className="text-xs text-muted-foreground mb-1">Meta Total</p>
              <p className="text-lg font-light tabular-nums">
                {formatCurrency(summaryData.totalTarget)}
              </p>
            </div>
            <div className="p-5 rounded-sm border bg-card">
              <p className="text-xs text-muted-foreground mb-1">Atingidas</p>
              <p className="text-lg font-light tabular-nums">
                {summaryData.achievedGoals}/{summaryData.count}
              </p>
            </div>
          </motion.div>
        )}
        ...
        <div className="flex items-center gap-1 border-b pb-0">
          ...
          {summaryData && (
            <span className="ml-auto text-[10px] text-muted-foreground">
              {summaryData.activeGoals} ativas · {summaryData.achievedGoals}{" "}
              atingidas
            </span>
          )}
        </div>
        ...
        {filteredGoals.length > 0 ? (
          <div className="space-y-3">
            {filteredGoals.map((goal: any, i: number) => {
              ...
              const monthsUntilTarget = goal.target_date
                ? Math.max(
                    0,
                    (new Date(goal.target_date).getFullYear() -
                      new Date().getFullYear()) *
                      12 +
                      new Date(goal.target_date).getMonth() -
                      new Date().getMonth(),
                  )
                : null;
              const projectedByTarget =
                monthsUntilTarget && goal.monthly_contribution > 0
                  ? goal.current_amount +
                      goal.monthly_contribution * monthsUntilTarget >=
                    goal.target_amount
                  : null;
              return (
                <motion.div
                ...
                >
                  <div className="p-4">
                    <div className="flex items-start gap-3">
                      <div
                        className={`w-9 h-9 rounded-sm flex items-center justify-center shrink-0 ${goal.is_achieved ? "bg-success/10" : "bg-secondary"}`}
                      >
                        <Icon
                          className={`w-4 h-4 ${goal.is_achieved ? "text-success" : "text-muted-foreground"}`}
                        />
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <span
                            className={`text-sm font-medium truncate ${goal.is_achieved ? "line-through text-muted-foreground" : ""}`}
                          >
                            {goal.name}
                          </span>
                          ...
                          {goal.is_achieved && (
                            <span className="text-[10px] px-1.5 py-0.5 rounded-sm bg-success/10 text-success font-medium">
                              Atingida ✓
                            </span>
                          )}
                        </div>
                      ...
                      <div className="text-right shrink-0">
                        <p
                          className={`text-lg font-light tabular-nums ${progress >= 100 ? "text-success" : ""}`}
                        >
                          {progress.toFixed(0)}%
                        </p>
                        {!goal.is_achieved && (
                    ...
                    <div className="mt-3">
                      <div className="h-2 bg-secondary rounded-full overflow-hidden">
                        <div
                          className={`h-full rounded-full transition-all duration-500 ${progress >= 100 ? "bg-success" : progress > 50 ? "bg-primary" : "bg-muted-foreground/30"}`}
                      ...
                      {monthsUntilTarget !== null &&
                        ...
                        !goal.is_achieved && (
                          <p
                            className={`text-[10px] mt-1 ${projectedByTarget ? "text-success" : "text-warning"}`}
                          >
                            {projectedByTarget
                              ? "Dentro do prazo"
                              : `Ritmo atual: ${monthsUntilTarget} meses restantes — precisa aumentar aporte`}
                          </p>
                        )}
                    </div>
                  </div>
        ...
        >
          <DialogContent className="sm:max-w-[320px]">
            <DialogHeader>
              ...
              <DialogDescription className="text-xs">
                {contributingGoal
                  ? `Quanto você quer guardar para "${contributingGoal.name}"?`
                  : ""}
              </DialogDescription>
            </DialogHeader>
            {contributingGoal && (
              <div className="space-y-4 py-2">
                <div className="bg-secondary/30 rounded-sm px-3 py-2 text-xs space-y-1">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-medium">
                      {formatCurrency(contributingGoal.current_amount)} /{" "}
                      {formatCurrency(contributingGoal.target_amount)}
                    </span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span>Faltam</span>
                    <span className="text-muted-foreground">
                      {formatCurrency(
                        contributingGoal.target_amount -
                          contributingGoal.current_amount,
                      )}
                    </span>
                  </div>
                </div>
                <div>
                  ...
                  <BRLCurrencyInput
                    ...
                    onChangeValue={setContributeAmount}
                    placeholder="R$ 0,00"
                    autoFocus
                  />
                </div>
