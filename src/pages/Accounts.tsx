...
function getAccountIcon(type: string) { return accountTypes.find((a) => a.value === type)?.icon || Landmark; }
    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div><h1 className="text-lg font-medium tracking-tight">Contas Bancárias</h1><p className="text-xs text-muted-foreground mt-1">Acompanhe o saldo das suas contas</p></div>
          <Dialog open={dialogOpen} onOpenChange={(open) => { setDialogOpen(open); if (!open) { resetForm(); setEditingAcc(null); } }}>
            ...
            <DialogContent className="sm:max-w-[380px]">
              <DialogHeader><DialogTitle className="text-sm font-medium">{editingAcc ? "Editar Conta" : "Nova Conta"}</DialogTitle><DialogDescription className="text-xs">Adicione uma conta bancária ou dinheiro físico</DialogDescription></DialogHeader>
              <div className="space-y-4 py-2">
                ...
                <div><label className="text-xs text-muted-foreground mb-1.5 block">Tipo</label><Select value={form.type} onValueChange={(v) => setForm({ ...form, type: v })}><SelectTrigger className="text-xs h-9"><SelectValue /></SelectTrigger><SelectContent>{accountTypes.map((t) => <SelectItem key={t.value} value={t.value} className="text-xs">{t.label}</SelectItem>)}</SelectContent></Select></div>
                ...
                <div><label className="text-xs text-muted-foreground mb-1.5 block">Cor</label><div className="flex gap-2 flex-wrap">{colorOptions.map((color) => (<button key={color} type="button" className={`w-7 h-7 rounded-sm border-2 transition-all ${form.color === color ? "border-foreground scale-110" : "border-transparent"}`} style={{ backgroundColor: color }} onClick={() => setForm({ ...form, color })} />))}</div></div>
              </div>
              <DialogFooter><Button variant="outline" size="sm" className="text-xs" onClick={() => { setDialogOpen(false); resetForm(); setEditingAcc(null); }}>Cancelar</Button>
                <Button size="sm" className="text-xs" onClick={async () => {
                  if (form.name && form.balance) {
                    ...
                    if (!useDemo) { if (editingAcc) await update(editingAcc.id, data); else await create(data); }
                    toast.success(editingAcc ? "Conta atualizada!" : "Conta criada!"); setDialogOpen(false); resetForm(); setEditingAcc(null);
                  }
                }}>{editingAcc ? "Salvar" : "Adicionar"}</Button></DialogFooter>
            </DialogContent>
          </Dialog>
        </div>
        ...
        {accounts.length > 0 ? (
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {accounts.map((acc: any, i: number) => {
              ...
              return (
                <motion.div key={acc.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                  ...
                  <div className="p-5" style={{ backgroundColor: `${acc.color}08` }}>
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex items-center gap-3">
                        <div className="w-9 h-9 rounded-sm flex items-center justify-center shrink-0" style={{ backgroundColor: acc.color }}><Icon className="w-4 h-4 text-white" /></div>
                        <div><p className="text-sm font-medium">{acc.name}</p><p className="text-xs text-muted-foreground">{getAccountLabel(acc.type)}</p></div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2 mt-4 pt-3 border-t opacity-0 group-hover:opacity-100 transition-opacity">
                      <Button variant="ghost" size="sm" className="text-[10px] h-7 px-2" onClick={() => { setEditingAcc(acc); setForm({ name: acc.name, type: acc.type, balance: String(acc.balance), color: acc.color }); setDialogOpen(true); }}><Pencil className="w-3 h-3 mr-1" />Editar</Button>
                      <Button variant="ghost" size="sm" className="text-[10px] h-7 px-2 text-destructive" onClick={() => { setDeletingId(acc.id); setDeleteDialogOpen(true); }}><Trash2 className="w-3 h-3 mr-1" />Excluir</Button>
                    </div>
                  </div>
                </motion.div>
              );
        ...
        <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
          <AlertDialogContent className="sm:max-w-[340px]">
            <AlertDialogHeader><AlertDialogTitle className="text-sm font-medium">Excluir conta?</AlertDialogTitle><AlertDialogDescription className="text-xs">Esta ação não pode ser desfeita.</AlertDialogDescription></AlertDialogHeader>
            <AlertDialogFooter>
              <AlertDialogCancel className="text-xs">Cancelar</AlertDialogCancel>
              <AlertDialogAction className="text-xs bg-destructive hover:bg-destructive/90" onClick={async () => { if (deletingId) { if (!useDemo) await remove(deletingId); toast.success("Conta excluída!"); } setDeleteDialogOpen(false); setDeletingId(null); }}>Excluir</AlertDialogAction>
            </AlertDialogFooter>
          </AlertDialogContent>
        </AlertDialog>
      </div>
