      ...
      <div className="max-w-2xl mx-auto space-y-8">
        ...
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="p-5 rounded-xl border bg-card">
          ...
          <div className="flex items-center justify-between py-2">
            <div className="flex items-center gap-2">{darkMode ? <Moon className="w-4 h-4" /> : <Sun className="w-4 h-4" />}<span className="text-sm">Modo Escuro</span></div>
            <button onClick={toggleDarkMode} className={`relative w-10 h-6 rounded-full transition-colors ${darkMode ? "bg-foreground" : "bg-secondary"}`}>
