...
import { Settings as SettingsIcon, User, Bell, Moon, Sun, Info } from "lucide-react";
      ...
      <div className="max-w-2xl mx-auto space-y-8">
        ...
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="p-5 rounded-sm border bg-card">
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="text-xs text-muted-foreground mb-1.5 block">Renda Mensal (opcional)</label>
              <Input type="number" step="0.01" min="0" value={monthlyIncome} onChange={(e) => setMonthlyIncome(e.target.value)} placeholder="Ex: 8000" />
            </div>
            <div>
              <label className="text-xs text-muted-foreground mb-1.5 block">Objetivo Financeiro</label>
              <Input value={financialGoal} onChange={(e) => setFinancialGoal(e.target.value)} placeholder="Ex: Reserva de emergência, comprar imóvel..." />
            </div>
        ...
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="p-5 rounded-sm border bg-card">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between py-2">
              ...
              >
                <div className={`absolute top-0.5 w-5 h-5 rounded-full bg-background border shadow-sm transition-transform ${darkMode ? "translate-x-[18px]" : "translate-x-0.5"}`} />
              </button>
            </div>
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="p-5 rounded-sm border bg-card">
          ...
          <div className="space-y-2 text-xs text-muted-foreground">
            ...
            <p>Versão 1.0.0</p>
            <p>100% gratuito. Seus dados são armazenados com segurança.</p>
          </div>
        </motion.div>
      </div>
    </DashboardLayout>
