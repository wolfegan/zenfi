...
export default function Investments() {
  ...
  const { data: realInvestments, loading: invLoading, getSummary, create, update, remove } = useInvestments();
  const [summary, setSummary] = useState<any>(null);

  useEffect(() => {
    if (!invLoading && realInvestments.length > 0) {
  ...
  return (
    <DashboardLayout>
      <div className="space-y-6">
            ...
            <DialogContent className="sm:max-w-[380px]">
              ...
              <DialogFooter><Button variant="outline" size="sm" className="text-xs" onClick={() => { setDialogOpen(false); resetForm(); }}>Cancelar</Button><Button size="sm" className="text-xs" onClick={async () => {
                if (form.name && form.amount && form.currentValue) {
                  if (!useDemo) { if (editingInv) await update(editingInv.id, { name: form.name, type: form.type as any, amount: parseFloat(form.amount), current_value: parseFloat(form.currentValue), monthly_contribution: parseFloat(form.monthlyContribution) }); else await create({ name: form.name, type: form.type as any, amount: parseFloat(form.amount), current_value: parseFloat(form.currentValue), monthly_contribution: parseFloat(form.monthlyContribution) }); }
        ...
        {summaryData && investments && investments.length > 0 && (
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="grid sm:grid-cols-3 gap-4">
            <div className="p-5 rounded-sm border bg-card"><p className="text-xs text-muted-foreground mb-1">Total Investido</p><p className="text-lg font-light tabular-nums">{summaryData.totalInvested.toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}</p></div>
            <div className="p-5 rounded-sm border bg-card"><p className="text-xs text-muted-foreground mb-1">Valor Atual</p><p className="text-lg font-light tabular-nums">{summaryData.totalCurrentValue.toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}</p></div>
            <div className="p-5 rounded-sm border bg-card"><p className="text-xs text-muted-foreground mb-1">Rentabilidade</p>
              <div className="flex items-center gap-1.5">{summaryData.returnPercentage >= 0 ? <TrendingUp className="w-4 h-4 text-success" /> : <TrendingDown className="w-4 h-4 text-destructive" />}<span className={`text-lg font-light tabular-nums ${summaryData.returnPercentage >= 0 ? "text-success" : "text-destructive"}`}>{summaryData.returnPercentage >= 0 ? "+" : ""}{summaryData.returnPercentage.toFixed(1)}%</span></div>
            </div>
          </motion.div>
        )}
        ...
        {investments && investments.length > 0 ? (
          <div className="space-y-2">
            {investments.map((inv: any, i: number) => {
              const returnPct = inv.amount > 0 ? ((inv.current_value - inv.amount) / inv.amount) * 100 : 0;
              const typeLabel = investmentTypes.find((t) => t.value === inv.type)?.label || inv.type;
              return (
                <motion.div key={inv.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }} className="flex items-center gap-4 p-4 rounded-sm border bg-card hover:shadow-sm transition-shadow group">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-0.5"><span className="text-xs text-muted-foreground">Investido: {inv.amount.toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}</span>{inv.monthly_contribution > 0 && <><span className="text-xs text-muted-foreground">·</span><span className="text-xs text-muted-foreground">+{inv.monthly_contribution.toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}/mês</span></>}</div>
                  </div>
                  <div className="text-right"><p className="text-sm tabular-nums">{inv.current_value.toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}</p><p className={`text-xs tabular-nums ${returnPct >= 0 ? "text-success" : "text-destructive"}`}>{returnPct >= 0 ? "+" : ""}{returnPct.toFixed(1)}%</p></div>
                  <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => { setEditingInv(inv); setForm({ name: inv.name, type: inv.type, amount: String(inv.amount), currentValue: String(inv.current_value), monthlyContribution: String(inv.monthly_contribution) }); setDialogOpen(true); }}><Pencil className="w-3.5 h-3.5" /></Button>
                    <Button variant="ghost" size="icon" className="h-7 w-7 text-destructive" onClick={() => { if (!useDemo) remove(inv.id); }}><Trash2 className="w-3.5 h-3.5" /></Button>
                  </div>
                </motion.div>
              );
            })}
