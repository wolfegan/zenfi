...
export default function Investments() {
  ...
  const investments = useQuery(api.investments.getAll);
  ...
  const resetForm = () => {
    setForm({ name: "", type: "stocks", amount: "", currentValue: "", monthlyContribution: "0" });
    setEditingInv(null);
  };
  ...
  const handleSubmit = async () => {
    ...
    if (isNaN(amount) || isNaN(currentValue)) return;
    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-lg font-medium tracking-tight">Investimentos</h1>
            ...
            <DialogContent className="sm:max-w-[380px]">
              ...
              <div className="space-y-4 py-2">
                <div>
                  ...
                  <Input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Ex: Tesouro Direto" />
                </div>
                <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-xs text-muted-foreground mb-1.5 block">Total Investido</label>
                    <Input type="number" step="0.01" min="0" value={form.amount} onChange={(e) => setForm({ ...form, amount: e.target.value })} />
                  </div>
                  <div>
                    <label className="text-xs text-muted-foreground mb-1.5 block">Valor Atual</label>
                    <Input type="number" step="0.01" min="0" value={form.currentValue} onChange={(e) => setForm({ ...form, currentValue: e.target.value })} />
                  </div>
                </div>
                <div>
              ...
              <DialogFooter>
                ...
                <Button size="sm" className="text-xs" onClick={handleSubmit}>{editingInv ? "Salvar" : "Adicionar"}</Button>
              </DialogFooter>
            </DialogContent>
          </Dialog>
        </div>
        ...
        {summary && investments && investments.length > 0 && (
          ...
          >
            ...
            <div className="p-5 rounded-sm border bg-card">
              ...
              <p className="text-lg font-light tabular-nums">
                {summary.totalCurrentValue.toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}
              </p>
            </div>
            <div className="p-5 rounded-sm border bg-card">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span className={`text-lg font-light tabular-nums ${summary.returnPercentage >= 0 ? "text-success" : "text-destructive"}`}>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] px-1.5 py-0.5 rounded-sm bg-secondary text-muted-foreground">{typeLabel}</span>
                    </div>
                    <div className="flex items-center gap-2 mt-0.5">
                      <span className="text-xs text-muted-foreground">
                        Investido: {inv.amount.toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}
                      </span>
                      {inv.monthlyContribution > 0 && (
                        <>
                          <span className="text-xs text-muted-foreground">·</span>
                          <span className="text-xs text-muted-foreground">
                            +{inv.monthlyContribution.toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}/mês
                          </span>
                        </>
                      )}
                    </div>
                  ...
                  <div className="text-right">
                    ...
                    <p className={`text-xs tabular-nums ${returnPct >= 0 ? "text-success" : "text-destructive"}`}>
                  ...
                  <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <Button variant="ghost" size="icon" className="h-7 w-7"
                      onClick={() => { setEditingInv(inv); setForm({ name: inv.name, type: inv.type, amount: String(inv.amount), currentValue: String(inv.currentValue), monthlyContribution: String(inv.monthlyContribution) }); setDialogOpen(true); }}>
                      <Pencil className="w-3.5 h-3.5" />
                    </Button>
                    <Button variant="ghost" size="icon" className="h-7 w-7 text-destructive" onClick={() => deleteInv({ id: inv._id })}>
