...
export default function CreditCardsPage() {
  ...
  useEffect(() => { if (!isLoading) setUseDemo(realCards === undefined); }, [isLoading, realCards]);
        ...
        <div className="flex items-center justify-between">
          ...
          <Dialog open={dialogOpen} onOpenChange={(open) => { setDialogOpen(open); if (!open) resetForm(); }}>
            ...
            <DialogContent className="sm:max-w-[380px]">
              ...
              <DialogFooter><Button variant="outline" size="sm" className="text-xs" onClick={() => { setDialogOpen(false); resetForm(); }}>Cancelar</Button><Button size="sm" className="text-xs" onClick={async () => { if (form.name && form.limit) { if (!useDemo) await createCard({ name: form.name, limit: parseFloat(form.limit), closingDay: parseInt(form.closingDay), dueDay: parseInt(form.dueDay), color: form.color }); setDialogOpen(false); resetForm(); }}}>Adicionar</Button></DialogFooter>
            </DialogContent>
          </Dialog>
        </div>
        ...
        {cards && cards.length > 0 ? (
          <div className="grid lg:grid-cols-2 gap-4">
            {cards.map((card: any, i: number) => {
              const spending = (card as any).bills?.reduce?.((s: number, b: any) => s + (b.isPaid ? 0 : b.totalAmount), 0) || 0;
              const utilization = card.limit > 0 ? (spending / card.limit) * 100 : 0;
              return (
                <motion.div key={card._id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }} className="rounded-sm border bg-card overflow-hidden">
                  <div className="p-5" style={{ backgroundColor: `${card.color}08` }}>
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div><p className="text-sm font-medium">{card.name}</p><p className="text-xs text-muted-foreground">Fechamento dia {card.closingDay} · Vence dia {card.dueDay}</p></div>
                      </div>
                      ...
                      {(card as any).bills.map((bill: any) => (
                        <div key={bill._id} className="flex items-center justify-between px-5 py-2.5">
                          <div className="flex items-center gap-2">
                            ...
                            <div><span className="text-xs">{new Date(bill.month + "-01").toLocaleDateString("pt-BR", { month: "long", year: "numeric" })}</span>{bill.isPaid && <span className="text-[10px] text-success ml-2">Pago</span>}</div>
                          </div>
                          <span className="text-xs tabular-nums">{bill.totalAmount.toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}</span>
                        </div>
                      ))}
                    </div>
                  )}
