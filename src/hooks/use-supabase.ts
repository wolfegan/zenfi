...
export function useCategories() {
  ...
  const create = useCallback(async (cat: Omit<Category, "id" | "user_id" | "created_at">) => {
  ...
  const update = useCallback(async (id: string, updates: Partial<Omit<Category, "id" | "user_id" | "created_at">>) => {
...
export function useTransactions() {
  ...
  const getByMonth = useCallback(async (month: string) => {
    if (!userId) return [];
    const { data: result } = await supabase
      .from("transactions")
      .select("*")
  ...
  const getByCreditCard = useCallback(async (creditCardId: string, month: string) => {
    const { data: result } = await supabase
      .from("transactions")
      .select("*")
  ...
  const create = useCallback(async (tx: Omit<Transaction, "id" | "user_id" | "created_at">) => {
    if (!userId) return null;
    const { data: result } = await supabase
      .from("transactions")
  ...
  const update = useCallback(async (id: string, updates: Partial<Omit<Transaction, "id" | "user_id" | "created_at">>) => {
  ...
  return { data, loading, getByMonth, getByCreditCard, create, update, remove, refetch: fetch };
}
  ...
  useEffect(() => {
    ...
    async function calc() {
      ...
      const [txRes, catRes, budgetRes] = await Promise.all([
        ...
        supabase.from("monthly_budgets").select("*").eq("user_id", userId).eq("month", month),
      ]);
      ...
      const fixedExpenses = monthTx.filter((t) => t.type === "expense" && t.is_fixed).reduce((s, t) => s + t.amount, 0);
      const variableExpenses = monthTx.filter((t) => t.type === "expense" && !t.is_fixed).reduce((s, t) => s + t.amount, 0);
      const creditCardExpenses = monthTx.filter((t) => t.is_credit_card).reduce((s, t) => s + t.amount, 0);

      const expensesByCategory = categories
        .map((cat) => ({
          category: cat,
          total: monthTx.filter((t) => t.category_id === cat.id && t.type === "expense").reduce((s, t) => s + t.amount, 0),
        }))
        .filter((c) => c.total > 0);
      ...
      const incomeByCategory = categories
        .map((cat) => ({
          category: cat,
          total: monthTx.filter((t) => t.category_id === cat.id && t.type === "income").reduce((s, t) => s + t.amount, 0),
        }))
        .filter((c) => c.total > 0);

      const budgetComparisons = budgets.map((budget) => {
        const spent = monthTx.filter((t) => t.category_id === budget.category_id && t.type === "expense").reduce((s, t) => s + t.amount, 0);
        return {
          budget,
          spent,
  ...
  useEffect(() => {
    ...
    async function calc() {
      ...
      const [txRes, budgetRes, ccRes, invRes] = await Promise.all([
        supabase.from("transactions").select("*").eq("user_id", userId),
        supabase.from("monthly_budgets").select("*").eq("user_id", userId).eq("month", currentMonth),
      ...
      if (creditCards.length > 0) {
        ...
        for (const card of creditCards) {
          const cardTotal = monthTx.filter((t) => t.credit_card_id === card.id).reduce((s, t) => s + t.amount, 0);
      ...
      const finalScore = Math.round(
        savingsScore * 0.25 + expenseScore * 0.25 + budgetScore * 0.25 + creditScore * 0.125 + invScore * 0.125
      );
...
export function useBudgets() {
  ...
  const setBudget = useCallback(async (categoryId: string, month: string, amount: number) => {
    if (!userId) return null;
    // Check if exists
...
export function useCreditCards() {
  ...
  const fetch = useCallback(async () => {
    ...
    if (!cards) { setData([]); setLoading(false); return; }
  ...
  const create = useCallback(async (card: Omit<CreditCard, "id" | "user_id" | "created_at">) => {
    if (!userId) return null;
    const { data: result } = await supabase
      .from("credit_cards")
  ...
  const update = useCallback(async (id: string, updates: Partial<Omit<CreditCard, "id" | "user_id" | "created_at">>) => {
  ...
  const createBill = useCallback(async (bill: Omit<CreditCardBill, "id" | "user_id" | "created_at">) => {
    if (!userId) return null;
    const { data: result } = await supabase
      .from("credit_card_bills")
  ...
  const toggleBillPaid = useCallback(async (id: string, isPaid: boolean) => {
    await supabase.from("credit_card_bills").update({ is_paid: isPaid }).eq("id", id);
  }, []);
  ...
  return { data, loading, create, update, remove, createBill, toggleBillPaid, deleteBill, refetch: fetch };
}
...
export function useInvestments() {
  ...
  const getSummary = useCallback(async (): Promise<InvestmentsSummary | null> => {
    if (!userId) return null;
  ...
  const create = useCallback(async (inv: Omit<Investment, "id" | "user_id" | "created_at">) => {
    if (!userId) return null;
    const { data: result } = await supabase
      .from("investments")
  ...
  const update = useCallback(async (id: string, updates: Partial<Omit<Investment, "id" | "user_id" | "created_at">>) => {
...
export function useDebts() {
  ...
  const create = useCallback(async (debt: Omit<Debt, "id" | "user_id" | "is_paid" | "created_at">) => {
    if (!userId) return null;
    const { data: result } = await supabase
      .from("debts")
  ...
  const update = useCallback(async (id: string, updates: Partial<Omit<Debt, "id" | "user_id" | "created_at">>) => {
    await supabase.from("debts").update(updates).eq("id", id);
    setData((prev) => prev.map((d) => (d.id === id ? { ...d, ...updates } : d)));
  }, []);

  const markAsPaid = useCallback(async (id: string) => {
    await supabase.from("debts").update({ is_paid: true, remaining_amount: 0 }).eq("id", id);
    setData((prev) => prev.map((d) => (d.id === id ? { ...d, is_paid: true, remaining_amount: 0 } : d)));
  }, []);
  ...
  const payInstallment = useCallback(async (id: string, amount: number) => {
    ...
    setData((prev) => prev.map((d) =>
      d.id === id ? { ...d, remaining_amount: newRemaining, is_paid: newRemaining <= 0 } : d
    ));
  }, [data]);
  ...
  return { data, loading, getActive, getSummary, create, update, markAsPaid, payInstallment, remove, refetch: fetch };
}
...
export function useGoals() {
  ...
  const create = useCallback(async (goal: Omit<Goal, "id" | "user_id" | "is_achieved" | "created_at">) => {
    if (!userId) return null;
    const { data: result } = await supabase
      .from("goals")
  ...
  const update = useCallback(async (id: string, updates: Partial<Omit<Goal, "id" | "user_id" | "created_at">>) => {
    await supabase.from("goals").update(updates).eq("id", id);
    setData((prev) => prev.map((g) => (g.id === id ? { ...g, ...updates } : g)));
  }, []);

  const markAsAchieved = useCallback(async (id: string) => {
    const goal = data.find((g) => g.id === id);
    if (!goal) return;
    await supabase.from("goals").update({ is_achieved: true, current_amount: goal.target_amount }).eq("id", id);
    setData((prev) => prev.map((g) =>
      g.id === id ? { ...g, is_achieved: true, current_amount: g.target_amount } : g
    ));
  }, [data]);
  ...
  const contribute = useCallback(async (id: string, amount: number) => {
    ...
    setData((prev) => prev.map((g) =>
      g.id === id ? { ...g, current_amount: newCurrent, is_achieved: newCurrent >= goal.target_amount } : g
    ));
  }, [data]);
  ...
  return { data, loading, getActive, getSummary, create, update, markAsAchieved, contribute, remove, refetch: fetch };
}
...
export function useAccounts() {
  ...
  const create = useCallback(async (acc: Omit<Account, "id" | "user_id" | "created_at">) => {
    if (!userId) return null;
    const { data: result } = await supabase
      .from("accounts")
  ...
  const update = useCallback(async (id: string, updates: Partial<Omit<Account, "id" | "user_id" | "created_at">>) => {
  ...
  return { data, loading, getTotalBalance, create, update, remove, refetch: fetch };
}
